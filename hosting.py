...
# ==================== OTHER STEP PROCESSORS ====================
def process_gift_prime(message):
    try:
        target_id = int(message.text.strip())
        add_prime_days(target_id, config['price_days'])
        bot.reply_to(message, f"🎉 **Successfully gifted {config['price_days']} days Prime VIP to `{target_id}`!**")
    except ValueError:
        bot.reply_to(message, "❌ **Invalid User ID!** Please enter numbers only.")

def process_claim_code(message):
    code = message.text.strip()
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT days FROM prime_codes WHERE code = ?", (code,))
    row = cursor.fetchone()
    ...
    if row:
        ...
        bot.reply_to(message, f"🎉 **Coupon Redeemed!** Added {days} days of Prime VIP subscription.")
    else:
        conn.close()
        bot.reply_to(message, "❌ **Invalid or expired coupon code!**")
...
def show_pip_action(chat_id, bot_id, msg_id):
    try:
        ...
        bot.send_message(chat_id, f"❌ Error fetching packages: `{str(e)}`")
...
def delete_bot_action(chat_id, bot_id, msg_id):
    ...
    conn.close()
    bot.send_message(chat_id, "🗑️ **Bot successfully deleted.**")
